...
export const adminModuleDefinitions: Record<AdminModuleKey, AdminModuleDefinition> = {
  ...
  orders: {
    ...
    fields: [
      ...
      { name: "payment_method", label: "Payment Method", type: "select", options: ["cod", "bank_transfer"].map((value) => ({ label: value, value })) },
...
async function createManualOrder(body: JsonRecord) {
  ...
  if (!productId) {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        total,
        payment_method: body.payment_method === "bank_transfer" ? "bank_transfer" : "cod",
  ...
  const input: CheckoutOrderInput = {
    ...
    paymentMethod: body.payment_method === "bank_transfer" ? "bank_transfer" : "cod",
    items: [
      {
        productId,
