-- Rana Velvet production setup
-- Supabase Dashboard > SQL Editor > run this whole file.
...
create policy "Public read active banners" on public.banners for select using (is_active = true and (start_date is null or start_date <= now()) and (end_date is null or end_date >= now()));
...
using (user_id = auth.uid() or exists (select 1 from public.admin_users au where au.user_id = auth.uid() and au."role" = 'super_admin' and au.is_active));
...
insert into public.categories (name, slug, image_url, sort_order, products_count)
values
  ('Home Decor', 'home-decor', 'https://images.unsplash.com/photo-1513519245088-0e12902e5a38?w=900&q=80', 1, 56),
  ('Furnishing Fabric', 'furnishing-fabric', 'https://images.unsplash.com/photo-1513694203232-719a280e022f?w=900&q=80', 2, 72),
  ('Cushions', 'cushions', 'https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=900&q=80', 3, 28),
  ('Smart Furniture', 'smart-furniture', 'https://images.unsplash.com/photo-1533090481720-856c6e5f6e2d?w=900&q=80', 4, 41),
  ('Prestige Furniture', 'prestige-furniture', 'https://images.unsplash.com/photo-1617806118233-18e1de247200?w=900&q=80', 5, 12)
on conflict (slug) do update set name = excluded.name, image_url = excluded.image_url, sort_order = excluded.sort_order, products_count = excluded.products_count;
